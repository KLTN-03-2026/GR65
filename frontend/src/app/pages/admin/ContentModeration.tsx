...
import { FileCheck, Briefcase, FileText, CheckCircle, XCircle, Eye, AlertCircle, Clock, Search, Filter } from "lucide-react";
...
export function ContentModeration() {
  ...
  const [activeTab, setActiveTab] = useState<"jd" | "cv">("jd");
  const [selectedItem, setSelectedItem] = useState<typeof mockPendingJDs[0] | null>(null);

  const approveJD = (id: string) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        {[
          { key: "jd" as const, label: "Bài đăng JD", count: jdItems.length },
          { key: "cv" as const, label: "CV ứng viên", count: cvItems.length },
        ].map(tab => (
        ...
        <div className="lg:col-span-2 space-y-3">
          {activeTab === "jd" ? (
            ...
            cvItems.length > 0 ? cvItems.map(cv => {
              const risk = riskConfig[cv.risk as keyof typeof riskConfig];
              return (
          ...
          {selectedItem && activeTab === "jd" ? (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
              <div className="flex items-start gap-4">
                ...
                <span className={`text-xs px-3 py-1.5 rounded-xl border ${riskConfig[selectedItem.risk as keyof typeof riskConfig].color}`}>
                  Nguy cơ: {riskConfig[selectedItem.risk as keyof typeof riskConfig].label}
                </span>
              </div>
